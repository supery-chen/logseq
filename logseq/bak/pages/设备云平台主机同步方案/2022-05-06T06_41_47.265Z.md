...
- | 字段     | 子字段                             | 类型          | 对应事件                 | 事件对应操作 | 备注 |
  ...
  | relation |                                    | object |                          |              |              |
  |          | user_count                         | int           | CCU_RELATION_CHANGED     | set |              |
  |          | users                              | list\<string> | CCU_RELATION_CHANGED     | set |              |
  |          | device_count                       | int           | CCU_RELATION_CHANGED     | set |              |
  |          | devices                            | list\<string> | CCU_RELATION_CHANGED     | set |              |
  |-||||||
- ## 打分
- ## 同步
