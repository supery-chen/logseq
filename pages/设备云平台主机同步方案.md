...
- | 字段     | 子字段                             | 类型          | 对应事件                 | 事件对应操作 | 备注 |
  ...
  | relation |                                    | object |                          |              | 如果收到的关联信息为空或空列表[:br]则会清空relation下所有数据 |
  |          | user_count                         | int           | CCU_RELATION_CHANGED     | set len(users) |              |
  |          | users                              | list\<string> | CCU_RELATION_CHANGED     | set users |              |
  |          | device_count                       | int           | CCU_RELATION_CHANGED     | set len(devs) |              |
