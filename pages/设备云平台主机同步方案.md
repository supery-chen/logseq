...
- ```java
  package com.superychen.score.service;
  
  import com.superychen.score.dao.RedisDao;
  import com.superychen.score.event.Event;
  import com.superychen.score.event.EventTypeEnum;
  import com.superychen.score.trace.Traces;
  import com.superychen.score.util.MdcUtil;
  import lombok.extern.slf4j.Slf4j;
  import reactor.core.publisher.Mono;
  
  /**
   * 主机待同步队列处理服务,负责将主机加入待同步队列与从待同步队列移除
   */
  @Slf4j
  public class SyncSortedQueueService {
  
      private static final String REDIS_KEY_SYNC_QUEUE = "superychen:sync_queue";
  
      /**
       * 主机更新或移出待同步队列,仅在主机上下线或同步失败时处理
       */
      public static Mono<Boolean> onEvent(Event event, Double score) {
          if (event == null) {
              return Mono.just(true);
          }
          return Mono.deferContextual(Traces::tid)
                  .flatMap(tid -> {
                      MdcUtil.updateTid(tid);
                      String ccuName = event.getCcuName();
                      EventTypeEnum eventType = event.getEventType();
                      //实际代码
                      if (eventType.isSync()) {
                          //主机上线,添加到队列
                          log.info("add [{}] {} to sync queue with score {}", eventType, ccuName, score);log.info("add [{}] {} to sync queue with score {}", eventType, ccuName, score);
                          return RedisDao.zadd(REDIS_KEY_SYNC_QUEUE, score, ccuName);
                      } else if (eventType == EventTypeEnum.CCU_OFFLINE) {
                          //主机离线,从队列移除
                          log.info("rem [{}] {} from sync queue", eventType, ccuName);
                          return RedisDao.zrem(REDIS_KEY_SYNC_QUEUE, ccuName);
                      }
                      //其它,不处理
                      log.debug("ignore [{}] {} score {}", eventType, ccuName, score);
                      return Mono.just(true);
                  })
                  .onErrorReturn(false);
      }
